...
class ItemView : FrameLayout {
    ...
    private fun generateViews(styles: TypedArray) {
        ...
        val textColor = styles.getColor(
            R.styleable.PinTextView_android_textColor,
            ResourcesCompat.getColor(context.resources, R.color.black, null)
        )
        val barHeight = styles.getDimension(R.styleable.PinTextView_bar_height, defaultHeight)
        val barMargin = styles.getDimension(
            R.styleable.PinTextView_bar_margin, Utils.getPixels(
                context, 0
            ).toFloat()
        )
        var barMarginBottom = styles.getDimension(
            R.styleable.PinTextView_bar_margin_bottom, DEFAULT_BAR_MARGIN.toFloat()
        )
        var barMarginRight = styles.getDimension(
            R.styleable.PinTextView_bar_margin_right, DEFAULT_BAR_MARGIN.toFloat()
        )
        var barMarginLeft = styles.getDimension(
            R.styleable.PinTextView_bar_margin_left, DEFAULT_BAR_MARGIN.toFloat()
        )
        var barMarginTop = styles.getDimension(
            R.styleable.PinTextView_bar_margin_top, DEFAULT_BAR_MARGIN.toFloat()
        )
        hideOTP = styles.getBoolean(R.styleable.PinTextView_hide_otp, false)
        hideOTPDrawable =
            styles.getResourceId(R.styleable.PinTextView_hide_otp_drawable, R.drawable.bg_pin)

        defaultOTPDrawable = ResourcesCompat.getColor(context.resources, R.color.transparent, null)

        val barEnabled = styles.getBoolean(R.styleable.PinTextView_bar_enabled, false)

        val otpTextSize =
            styles.getDimension(R.styleable.PinTextView_otp_text_size, defaultOtpTextSize)

        val otpTextTypeFace = styles.getString(R.styleable.PinTextView_text_typeface)
        val boxBackgroundColor = styles.getResourceId(
            R.styleable.PinTextView_otp_box_background,
            ResourcesCompat.getColor(context.resources, R.color.transparent, null)
        )
        boxBackgroundColorActive = styles.getResourceId(
            R.styleable.PinTextView_otp_box_background_active, boxBackgroundColor
        )
        boxBackgroundColorInactive = styles.getResourceId(
            R.styleable.PinTextView_otp_box_background_inactive, boxBackgroundColor
        )
        boxBackgroundColorSuccess = styles.getResourceId(
            R.styleable.PinTextView_otp_box_background_success, boxBackgroundColor
        )
        boxBackgroundColorError = styles.getResourceId(
            R.styleable.PinTextView_otp_box_background_error, boxBackgroundColor
        )
        barActiveColor = styles.getColor(
            R.styleable.PinTextView_bar_active_color,
            ResourcesCompat.getColor(context.resources, R.color.black, null)
        )
        barInactiveColor = styles.getColor(
            R.styleable.PinTextView_bar_inactive_color,
            ResourcesCompat.getColor(context.resources, R.color.grey, null)
        )
        barErrorColor = styles.getColor(
            R.styleable.PinTextView_bar_error_color,
            ResourcesCompat.getColor(context.resources, R.color.red, null)
        )
        barSuccessColor = styles.getColor(
            R.styleable.PinTextView_bar_success_color,
            ResourcesCompat.getColor(context.resources, R.color.black, null)
        )
